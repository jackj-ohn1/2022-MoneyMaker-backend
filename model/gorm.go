...
func GetLastRecord() (tables.Good, error) {
	...
	err := mysql.DB.Model(&tables.Good{}).Order("goods_id desc").Find(&good).Error
	return good, err
}
...
func GetUserAllGoods(id string) ([]tables.Good, error) {
	var goods []tables.Good
	err := mysql.DB.Model(&tables.Good{}).Where("id=? AND goodsin='yes'", id).Find(&goods).Error
	return goods, err
}
...
func UpadateName(name string, id string) error {
	err := mysql.DB.Model(&tables.User{}).Where("id=?", id).Update("nickname", name).Error
	return err
}
